...
def write_to_db():
    ...
    while True:
        ...
        try:
            cur.execute("""INSERT INTO `temperature_at_all_times` (`Date`, `Time`, `Temperature`) VALUES (%s,%s,%s)""", (dateWrite,timeWrite,temperature))
            db.commit()
            result= cur.fetchone()
            print db
            print "\nProcess finished"
...
def write_to_cloud():
  # Get data from server
    ...
    try:
        ...
        print (temperature + ',' + timeWrite + ',' + dateWrite)
        #insert record
